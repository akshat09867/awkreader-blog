...
To do this, I started writing formal unit tests using the `testthat` package, taking reference from the example testing files. However, when I ran `test_that()`, I ran into two really interesting issues related to our AWK translation engine and data types.

**Issue 1: The AWK Translation Engine Quoting Bug**
...
**Issue 2: The `fread` Data Type Trap**
...
* **Architecture note for later:** I plan to update `filtered.fread` so it automatically preserves the original column types. I can do this by running a quick, 0-row read of the original file (`data.table::fread(file, nrows = 0)`) to extract the correct classes, and then pass that vector into the `colClasses` argument of the main `fread` call.
